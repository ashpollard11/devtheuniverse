let InternalComponent = Vue.component("internal", {
	...
	template:
		`<div class="content" v-on:pointermove="onMouseMove">
			<div class="wrapper">
				<svg class="frame frame1" ref="text_anim">
					<path id="morph_frame" ref="morph_frame" class="morph-frame" :d="morph_d_attr" stroke-miterlimit="10" />
					<path id="curve1" ref="curve1" class="curve curve1" :d="d_attr" stroke-miterlimit="10" />
					<text x="0">
	...
	methods: {
		animateSequence: function(e) {
			...
			let textPathArr = Array.from(path.textContent);
			let delay = "2s";
			let spacing = 40;
			let duration = 4;
		...
		onMouseMove: function(e) {
			...
			TweenMax.delayedCall(5, () => {
				if (this.currentPos.x === e.x && this.currentPos.y === e.y) {
					...
					this.morphTl = new TimelineMax({ repeat:-1, yoyo: true, repeatDelay: 1.5, onComplete: () => { this.morphTl.clear(); }, onRepeat: () => {
						// this.morphFrames.push(this.morphFrames.shift());
						// this.morph_d_attr = this.morphFrames[0];
					} });

					this.morphTl.to("#curve1", 1.5, {morphSVG: "#morph_frame", ease: Back.easeOut})
						.to("#morph_frame", 1.5, {morphSVG: "#curve1", ease: Back.easeOut});
				} else {
					this.morphTl.clear();
				}
			})
			...
			this.$refs.curve1.setAttribute("d", this.solve(this.points, 1.5));
			if (this.points.length > 210 && this.$refs.curve1_textpath.getComputedTextLength() >= 1110) this.points.splice(0, 6);
