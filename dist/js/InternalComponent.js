...
var InternalComponent = Vue.component("internal", {
	...
	methods: {
		animateSequence: function animateSequence(e) {
			...
			var pathArr = Array.from(path.textContent);
			var delay = 100;
			var spacing = 40;
			var duration = 0.5;
			...
			pathArr.forEach(function (letter, i) {
				var str = '';

				if (letter.trim() !== ' ') {
					str += '<tspan style=" animation-delay:' + delay + 'ms;' + '-webkit-animation-delay:' + delay + 'ms; ">' + letter + '<animate attributeType="XML" attributeName="x" to="' + spacing + '" dur="' + duration + 's" repeatCount="1"/></tspan>';
					delay += 50;
					spacing += 40;
					duration += 0.02;
				} else {
		...
		onMouseMove: function onMouseMove(e) {
			...
			this.$refs.curve2.setAttribute("d", this.solve(this.points, 1.5));
			// this.d_attr +=  "," + this.solve(this.points);

			var tl = this.pathTL;
			var stroke = this.stroke;
			this.strokes.reduce(function (tl, stroke) {
				var time = stroke.elapsed / 1000;
				var draw = stroke.dist / totalDist * 100 + "%";

				// this.complete = false;

				// return tl.to(this.$refs.curve2, time, { drawSVG: draw, onComplete: () => {
				// 	this.complete = true;
		...
		solve: function solve(data, k) {
			...
			for (var i = 0; i < size - 2; i += 2) {
				...
				var cp1x = (x1 + (x2 - x0) / 6 * k).toFixed(3);
				var cp1y = (y1 + (y2 - y0) / 6 * k).toFixed(3);

				var cp2x = (x2 - (x3 - x1) / 6 * k).toFixed(3);
				var cp2y = (y2 - (y3 - y1) / 6 * k).toFixed(3);
